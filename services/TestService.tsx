...
export const TestService = {
  ...
  testSimpleMessage: async (): Promise<void> => {
    ...
    if (result.success) {
      ...
      Alert.alert(
        '❌ Error',
        result.error || 'Error al obtener mensaje'
      );
    }
  },
  ...
  testEncryption: async (): Promise<void> => {
    ...
    const result = await apiRequests.post(
      endpoints.test.testEncryption(),
      testData
    );
    ...
    if (result.success) {
      Alert.alert(
        '🔐 Cifrado OK',
        'Comunicación segura funcionando correctamente'
      );

    } else {
      Alert.alert(
        '❌ Error Cifrado',
        result.error || 'Fallo en cifrado/descifrado'
      );
    }
  },
  ...
  runAllTests: async (): Promise<void> => {
    ...
    try {
      ...
      Alert.alert(
        '❌ Error Crítico',
        'Ocurrió un error inesperado durante las pruebas'
      );
    }
  }
};